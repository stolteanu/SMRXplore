...
def _period_subtitle(finess: str, periods: list[dict]) -> str:
    ...
    Mois affichés en notation ATIH "M0N" (jargon métier, cf. docstring module
    et _last_week_of_month) plutôt qu'en toutes lettres — demande utilisateur
    2026-09-16, la notation en toutes lettres ("août") laissait croire que le
    mois calendaire complet était transmis. Le mois de fin utilise
    `period["month"]` (mois ATIH, règle du jeudi), PAS le mois calendaire de
    `period["end"]` (dimanche de la dernière semaine transmise) : un envoi
    M07 peut se terminer un dimanche de début août (la date de transmission
    est calée sur la fin de semaine, pas sur la fin de mois) — utiliser
    `end.month` affichait alors à tort "M08" (2026-09-16)."""
    months = [p["month"] for p in periods]
    years = [p["year"] for p in periods]
    mois_debut = "M01"
    mois_fin = f"M{max(months):02d}"
...
# de petits pourcentages). Plafonné à TREND_CAP_PCT (= l'échelle -100%/+100%
# affichée à l'utilisateur, corrigé 2026-09-11 : l'ancien plafond à 15%
# rendait un écart de 10% presque aussi foncé qu'un écart de 60%) : au-delà,
# l'opacité du fond dégradé sature. Couleurs FIXES ici (pas de sélecteur live
# comme dans l'Explorateur ; la personnalisation de page, elle, est couverte
# par `apparence`/_apparence_style, séparément). Le on/off, lui, EST live
# (bouton "Couleurs de tendance" du header, cf. trend_legend (bouton inclus)) : les
# cellules portent toujours leurs classes/variables CSS, un simple toggle de
# classe sur .viz-root masque le dégradé sans regénérer le fichier.
TREND_CAP_PCT = 100.0
...
def _trend_delta(cur, prev, is_pct: bool = False) -> float | None:
    ...
    if is_pct:
        return cur - prev
    if prev == 0:
        # Division par zéro impossible mathématiquement : une apparition/disparition
        # depuis 0 est la plus grosse variation possible, pas une absence de variation
        # (l'ancien "return None" masquait ces cas au lieu de les signaler au max).
        if cur == 0:
            return None
        return TREND_CAP_PCT if cur > 0 else -TREND_CAP_PCT
...
def _trend_td(formatted: str, delta: float | None) -> str:
    ...
    qu'une valeur de comparaison existe). Le dégradé est posé via la classe
    "trend-cell" + les variables CSS --tc (couleur)/--ta (alpha) plutôt qu'un
    style inline figé (cf. TREND_CAP_PCT) : ça permet au bouton on/off du
    header (trend_legend (bouton inclus)) de masquer tout le dégradé d'un coup via une
    seule classe sur .viz-root, sans toucher à chaque cellule."""
    ...
    style = f"--tc:{color}; --ta:{alpha:.3f};"
    return f'<td class="trend-cell" style="{style}"><span class="trend-arrow" style="color:{color}">{arrow}</span> {formatted}</td>'
...
def _populate_notes(notes: "NoteCollector") -> dict[str, str]:
    ...
    m["indic_warn"] = notes.add(
        "warn",
        "Nb CSAR(R) = réalisations CSARR + CSAR (plafond 2 occurrences identiques/jour). "
    ...
    m["activite_score"] = notes.add(
        "warn",
        "Score de réadaptation GLOBALE = pondérations ATIH de TOUS les actes CSARR+CSAR+CCAM réalisés (modulateur "
        "de lieu inclus pour le CSARR et le CSAR). SPÉCIALISÉE = même somme, restreinte aux seuls actes marqueurs "
        "du GN du séjour. Décomposés par intervenant sur demande, alors que ce sont officiellement des indicateurs "
        ...
        "ou collectif selon <code>nombre_reel_patients</code>. Un acte CSAR est d'abord transcodé en acte CSARR "
        "de référence pour le couple (code, intervenant, modalité collective) — <code>nomenclature_csar_transcodage_detail</code>, "
        "ATIH <code>CSAR_infos.xlsx</code> annexe 7 — puis sa pondération retenue est le MAX entre celle du "
        "modulateur de temps CSAR (T0-T4) et celle de l'acte CSARR transcodé, majorée par le modulateur de lieu "
        "CSAR propre (L1/L2/L3, cf. Manuel des GME vol.1 §3.3.1.3-3.3.1.4) — voir "
...
def render(
    ...
    # intervenant dans le RHS, leur contribution n'est PAS mélangée à cette
    # table par intervenant (2026-09-18, demande utilisateur — retiré du
    # rendu, chantier de sous-table CCAM dédiée noté à part, voir mémoire
    # pmsi_smr_chantier_sous_table_ccam_section5) ; elle reste néanmoins
    # comprise dans les totaux GLOBALE/SPÉCIALISÉE en pied de table, qui
    # somment score_by_code_year SANS filtrer sur codes_sorted, pour ne pas
    # sous-évaluer le score officiel ATIH.
    ...
    for y in years:
        ...
        # Écart : colorée comme toute autre colonne, en comparant à l'ANNÉE
        # PRÉCÉDENTE (pas par son propre signe — corrigé 2026-09-11 : un essai
        # précédent coloriait en rouge tout écart négatif quelle que soit son
        # évolution, donc un écart qui se RÉSORBE d'une année à l'autre, ex.
        # -569 955 € -> -41 754 €, ressortait quand même en rouge alors que
        # c'est une amélioration — td()/_trend_delta gère déjà correctement
        # les valeurs négatives, cf. leur formule symétrique en abs(prev)).
        ecart_prev = (vp["montant_br_tot"] - vp["montant_br_pt_avec_estimation"]) if vp else None
        ecart_cell = td(ecart, fmt(ecart, 2, " €"), ecart_prev)
    ...
    if signalisation_couleur:
        trend_legend = (
            ...
            '<button type="button" class="trend-toggle-btn" id="trend-toggle-btn" '
            'onclick="smrToggleTrend()">Couleur de tendance : ON</button>'
            "</div>"
            "<script>(function(){"
            "var root=document.querySelector('.viz-root');"
            "var btn=document.getElementById('trend-toggle-btn');"
            # Pas de mémorisation (ni localStorage, ni autre) : le comportement
            # implicite d'un TDB généré avec la case "signalisation couleur"
            # cochée doit TOUJOURS être ON à l'ouverture, jamais hérité d'un
            # OFF cliqué sur un autre TDB (corrigé 2026-09-11 — l'ancienne
            # version utilisait une clé localStorage partagée entre TOUS les
            # TDB générés, donc un OFF sur un fichier "contaminait" les
            # suivants). Le bouton reste un toggle purement local à la page
            # ouverte, réinitialisé à ON à chaque (re)chargement.
            "function apply(off){"
            "root.classList.toggle('trend-off',off);"
            "if(btn)btn.textContent='Couleur de tendance : '+(off?'OFF':'ON');"
            "}"
            "window.smrToggleTrend=function(){"
            "apply(!root.classList.contains('trend-off'));"
            "};"
            "})();</script>"
        )
...
def render_annexe(data: dict) -> str:
    ...
    csar_ignores = data["csar_actes_ignores"]
    csar_ignores_rows = "".join(
        f"<tr><td>{periods_by_year[a['annee']]['label']}</td><td>{a['numero_admin_sejour']}</td>"
        f"<td>{a['date_realisation'] or '—'}</td><td>{a['code_csar']}</td><td>{a['intervenant']}</td>"
        f"<td>{a['modalite_collective']}</td><td>{fmt_int(a['nombre_realisations'])}</td></tr>"
        for a in csar_ignores
    )
    if not csar_ignores:
        csar_ignores_rows = '<tr><td colspan="7" style="text-align:center;color:var(--muted)">Aucun acte ignoré</td></tr>'
    ...
    body = ANNEXE_TEMPLATE.format(
        ...
        csar_ignores_rows=csar_ignores_rows,
    )
...
STYLE_BLOCK = """
  ...
  /* --row-bg (2026-09-11, correctif dégradé de tendance) : le fond "de base"
     de la ligne, redéfini par les lignes de total/regroupement ci-dessous —
     td.trend-cell mélange sa couleur de tendance PAR-DESSUS cette variable
     (pas --surface en dur) pour ne jamais écraser le gris des totaux, que le
     dégradé soit affiché ou masqué par le bouton on/off. */
  table { width: 100%; border-collapse: collapse; font-size: 13px; background: var(--surface); --row-bg: var(--surface); }
  table th, table td { padding: 7px 10px; text-align: right; border-bottom: 1px solid var(--grid); font-variant-numeric: tabular-nums; background: var(--row-bg); color: var(--ink); }
  ...
  table tfoot td, table tfoot th, table tr.total-row td {
    ...
    --row-bg: color-mix(in srgb, #000 14%, var(--surface)); background: var(--row-bg);
  }
  ...
  table tr.group-row td { --row-bg: color-mix(in srgb, #000 14%, var(--surface)); background: var(--row-bg); text-align: left; font-size: 12px; }
  ...
  /* Signalisation couleur (option avant génération, 2026-09-12) : dégradé de
     fond posé via les variables --tc (couleur)/--ta (alpha), propres à
     chaque cellule (_trend_td) — la classe ne fait que consommer ces
     variables, ce qui permet au bouton on/off (2026-09-11, trend_legend (bouton inclus))
     de tout masquer d'un coup avec .viz-root.trend-off, sans regénérer le
     fichier. */
  td.trend-cell { background: color-mix(in srgb, var(--tc) calc(var(--ta) * 100%), var(--row-bg)); }
  .viz-root.trend-off td.trend-cell { background: var(--row-bg); }
  .viz-root.trend-off td.trend-cell .trend-arrow { display: none; }
  .trend-arrow { font-size: 0.85em; margin-right: 2px; }
  .trend-legend { display: flex; align-items: center; margin: 0 0 14px; }
  .trend-toggle-btn {
    flex: none; background: none; color: var(--ink-2); border: 1px solid var(--border); border-radius: 999px;
    padding: 3px 12px; font-size: 11.5px; font-weight: 600; cursor: pointer; font-family: inherit;
  }
  .trend-toggle-btn:hover { filter: brightness(1.15); }
  @media print {
    .trend-toggle-btn { display: none !important; }
  }
...
GUIDE_TDB_HTML = """
<p class="guide-intro">Ce tableau de bord présente, pour un établissement (identifié par son FINESS) et sur
plusieurs périodes comparables, les principaux chiffres d'activité et de valorisation financière d'un service
de Soins Médicaux et de Réadaptation (SMR, anciennement SSR). Les périodes comparées sont toujours calées sur
les mêmes semaines calendaires (semaine 01 à N), jamais sur des années civiles complètes, pour que la
comparaison entre années reste valable même en cours d'année.</p>
<p class="guide-intro">Il est composé de 9 sections :</p>
<dl>
  <dt>1 · Patients</dt><dd>effectifs et âge moyen par sexe.</dd>
  <dt>2 · Séjours</dt><dd>volume d'activité (séjours, RHS, journées de présence, DMH, NbLits moy, EXH).</dd>
  <dt>3 · Journées de présence par semaine</dt><dd>comparaison semaine par semaine des journées de présence
  entre années.</dd>
  <dt>4 · Indicateurs</dt><dd>scores AVQ, actes CSAR(R)/diagnostics et leurs ratios moyens.</dd>
  <dt>5 · Activité CSARR par intervenant</dt><dd>actes réalisés et score de réadaptation par métier.</dd>
  <dt>6 · Valorisation</dt><dd>montants de remboursement, PMCT/PMST/PMJT, suppléments en sus, séjours non
  valorisés.</dd>
  <dt>7-8 · Palmarès des pathologies prises en charge</dt><dd>classement des 5 types de prise en charge les
  plus fréquents (CM et GN).</dd>
  <dt>9 · Caractéristiques des GME (GR / GL / Sévérité)</dt><dd>répartition de tous les séjours selon ces
  trois axes.</dd>
</dl>

<h3>1 · Patients</h3>
<p class="guide-intro">Combien de patients différents ont été pris en charge sur la période, et leur
répartition par sexe et par âge.</p>
<dl>
  ...
  donc parfois partagé entre plusieurs personnes d'un même foyer). Un même patient n'est compté qu'une seule
  fois sur la période, même s'il a eu plusieurs séjours.</dd>
  <dt>% F / % M / % Total</dt><dd>Part de chaque sexe dans le total de la période (% Total vaut toujours
  100 %, rappelé pour la lisibilité).</dd>
  <dt>Âge moy. F / M / Total</dt><dd>Âge moyen à la date d'entrée du séjour.</dd>
</dl>
...
<dl>
  ...
  dans l'année).</dd>
  <dt>Nb RHS</dt><dd>Nombre de résumés hebdomadaires standardisés transmis : une ligne par semaine de séjour
  transmise (un séjour de 6 semaines produit 6 RHS).</dd>
  ...
  période.</dd>
  <dt>NbLits moy</dt><dd>Nombre moyen de lits occupés, obtenu en divisant le nombre de journées de présence
  par (le nombre de semaines RHS effectivement transmises sur la période × 7).</dd>
  <dt>EXH</dt><dd>Taux d'exhaustivité : part des semaines transmises (RHS) sans erreur de groupage bloquante
  sur la période, en pourcentage.</dd>
</dl>

<h3>3 · Journées de présence par semaine</h3>
<p class="guide-intro">Le graphique compare, semaine ISO par semaine ISO (01, 02, 03…), le nombre de journées
de présence de chaque année sélectionnée — une courbe par année, distinguée par couleur et style de trait pour
rester lisible en impression noir et blanc. L'échelle verticale de gauche est <b>relative à la plage des
valeurs observées</b> (l'axe ne part pas de zéro), afin de rendre visibles les variations d'une semaine à
l'autre ; les graduations affichent les valeurs réelles pour permettre de lire l'amplitude exacte des écarts.
L'axe de droite convertit la même échelle en <b>nombre de lits</b> (journées de présence divisées par 7).
Trois lignes de repère horizontales indiquent la moyenne (trait plein), le minimum et le maximum de la
période (pointillés).</p>

<h3>4 · Indicateurs</h3>
<p class="guide-intro">Indicateurs médicaux et d'activité complémentaires, en moyenne ou en cumul sur la
période.</p>
<dl>
  <dt>AVQ phys. moy. / AVQ cogn. moy.</dt><dd>Moyenne, par semaine transmise (RHS), des scores de dépendance
  physique et cognitive (activités de la vie quotidienne) — plus le score est élevé, plus la dépendance est
  importante.</dd>
  <dt>Nb CSAR(R)</dt><dd>Nombre d'actes de rééducation-réadaptation réalisés sur la période, en additionnant
  les nomenclatures CSARR et CSAR (le second remplace progressivement le premier depuis 2026, certains
  établissements codant encore partiellement dans l'ancien catalogue), en évitant de compter plusieurs fois
  un doublon de transmission (un même acte transmis 3 fois ou plus le même jour n'est compté que 2 fois, ce
  qui correspond à une réalisation matin et après-midi).</dd>
  <dt>Nb diag.</dt><dd>Nombre de diagnostics associés distincts (couples semaine RHS/code diagnostic), une
  fois les doublons de transmission retirés au sein d'une même semaine.</dd>
  <dt>Nb moy. DAS/RHS</dt><dd>Nombre moyen de diagnostics associés par semaine transmise (RHS).</dd>
  <dt>Nb moy. interv./RHS (approx.)</dt><dd>Nombre moyen d'actes par semaine transmise (RHS), en additionnant
  les actes codés CSARR et CSAR — indicateur approximatif, à confirmer.</dd>
  <dt>Nb moy. CSAR(R)/j</dt><dd>Nombre moyen d'actes CSARR + CSAR par journée de présence.</dd>
</dl>
...
ergothérapeute, etc. — identifiés par leur code intervenant CSARR officiel), avec les périodes comparées côte
à côte pour chaque métier.</p>
<dl>
  <dt>Nb réalisations</dt><dd>Comptage brut du nombre d'actes CSARR réalisés par ce métier, sans aucun
  retraitement — méthode vérifiée à l'identique du rapport officiel ATIH correspondant. Ne compte que les
  actes codés CSARR (pas CSAR).</dd>
  <dt>Score de réadaptation globale</dt><dd>Chaque acte de rééducation-réadaptation réalisé par ce métier
  (CSARR, ou CSAR transcodé en équivalent CSARR pondéré) est valorisé par une pondération officielle ATIH,
  majorations de lieu comprises, puis ces valorisations sont additionnées, cumulées sur la période. C'est la
  définition officielle du score "global", décomposée ici par métier plutôt que laissée au niveau du
  séjour.</dd>
  <dt>Score de réadaptation spécialisée</dt><dd>Même pondération que le score global, mais comptée en plus
  dans ce score uniquement si l'acte est un marqueur reconnu de la pathologie du séjour (liste ATIH par
  groupe nosologique). Certains types de pathologie n'ont aucune liste de marqueurs définie par l'ATIH : leurs
  séjours ont alors un score spécialisé toujours nul. Le score spécialisé est donc toujours inférieur ou égal
  au score global.</dd>
</dl>
...
<dl>
  ...
  attribué en bloc aux séjours dont toute l'activité de la période est déjà terminée (pas de répartition par
  jour) ; hors transport, molécules onéreuses et supplément cancérologie (voir "suppléments en sus"
  ci-dessous) et hors séjours en anomalie de facturation (chaînage, en attente de droits, non facturables à
  l'Assurance Maladie).</dd>
  <dt>Montant BR estimé PRT</dt><dd>Une reconstitution du même montant, cette fois répartie au prorata des
  journées réellement présentes de chaque séjour dans la période, complétée par une estimation du montant des
  séjours encore en cours (non facturés car pas encore clôturés). Indicateur à titre indicatif, pas un chiffre
  officiel ATIH — un grand écart en cours d'année est normal (année pas terminée), pas une anomalie.</dd>
  <dt>Écart</dt><dd>Montant BR SÉJOUR (fact.) moins Montant BR estimé PRT.</dd>
  <dt>PMCT</dt><dd>prix moyen par cas traité : montant moyen par séjour.</dd>
  <dt>PMST</dt><dd>prix moyen par semaine traitée : montant moyen par semaine transmise (RHS).</dd>
  <dt>PMJT</dt><dd>prix moyen par journée traitée : montant moyen par journée de présence.</dd>
</dl>
<p class="guide-intro">PMCT, PMST et PMJT sont calculés à partir de la reconstitution prorata temporis
(Montant BR estimé PRT), sans la part estimée des séjours en cours — pas à partir du montant officiellement
facturé.</p>
...
n'ont encore aucun montant BR connu alors qu'ils "devraient" déjà en avoir un : anomalie de chaînage, attente
de droits à l'Assurance Maladie, non-facturabilité à l'Assurance Maladie, erreur de groupage, ou simplement
séjour encore en cours (moins de 90 jours, pas encore clôturé).</p>
<p class="guide-intro"><b>Ventilation par UF ou par type d'hospitalisation</b> : quand ce tableau est affiché
filtré par unité médicale ou par type d'hospitalisation, la répartition du Montant BR SÉJOUR (fact.) ne suit
pas la même méthode selon l'axe : par type d'hospitalisation (HC/HTP), une colonne source dédiée indique
directement le type de chaque ligne de facturation, donc aucune répartition n'est nécessaire. Par UF, en
revanche, aucune colonne équivalente n'existe dans les données de facturation : le montant officiel du séjour
est alors réparti au prorata de ses journées de présence par UF, <i><u>sur l'ensemble du séjour</u></i> (pas
seulement ses journées dans la période affichée) — par exemple, un séjour de 100 jours dont 80 en UF A et 20
en UF B se voit attribuer 80&nbsp;%/20&nbsp;% du montant facturé, quelle que soit la période consultée. Cette
répartition par UF est donc différente de celle du Montant BR estimé PRT, qui ne compte que les journées de
présence dans la période étudiée.</p>

<h3>7 · Palmarès des pathologies prises en charge (CM)</h3>
<p class="guide-intro">Classement des 5 Catégories Majeures (CM) les plus fréquentes, en nombre de séjours et
en montant financier. Le classement (mêmes 5 codes) est déterminé sur l'effectif cumulé de toutes les
périodes comparées, et reste identique d'une période à l'autre même si le rang d'un code change — pour
permettre une comparaison directe.</p>
<dl>
  <dt>Code — Libellé</dt><dd>Code officiel de la catégorie majeure et son intitulé.</dd>
  ...
  <dt>Valorisation</dt><dd>Montant BR séjour (hors suppléments) rattaché à ce code, calculé de la même façon
  que le Montant BR SÉJOUR (fact.) de la section 6 (attribué en bloc, pas de répartition par jour).</dd>
  <dt>% (sous chaque valeur)</dt><dd>Part de ce code dans le total, tous codes confondus, de sa colonne.</dd>
</dl>

<h3>8 · Palmarès des pathologies prises en charge (GN)</h3>
<p class="guide-intro">Même principe que la section 7, mais classé par Groupe Nosologique (GN) plutôt que par
Catégorie Majeure.</p>

<h3>9 · Caractéristiques des GME (GR / GL / Sévérité)</h3>
<p class="guide-intro">Répartition de tous les séjours (pas seulement un top 5) selon trois axes de
classification qui composent le code GME, toutes catégories de prise en charge confondues.</p>
<dl>
  <dt>GR (Groupe de Rééducation)</dt><dd>Type de rééducation dont relève le séjour.</dd>
  <dt>GL (Groupe de Lourdeur)</dt><dd>Niveau de lourdeur du séjour, déterminé à partir de la dépendance
  physique, de la dépendance cognitive, de l'âge et du caractère post-opératoire du séjour ; figé à "Niveau A"
  pour tous les séjours HTP.</dd>
...
HTML_TEMPLATE = """<div class="viz-root">
  ...
  <section>
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th>Période</th><th>AVQ phys. moy.</th><th>AVQ cogn. moy.</th><th>Nb CSAR(R)</th><th>Nb diag.</th><th>Nb moy. DAS/RHS</th><th>Nb moy. interv./RHS (approx.)</th><th>Nb moy. CSAR(R)/j</th></tr></thead>
...
ANNEXE_TEMPLATE = """<div class="viz-root">
  ...
  <section>
    <h2>3 · Actes CSAR ignorés (non transcodables)</h2>
    <div class="table-wrap">
      <table>
        <thead><tr><th>Période</th><th>N° admin séjour</th><th>Date</th><th>Code CSAR</th><th>Intervenant</th><th>Modalité codée</th><th>Nb réalisations</th></tr></thead>
        <tbody>{csar_ignores_rows}</tbody>
      </table>
    </div>
    <div class="callout warn"><span class="ico">i</span><div>Actes CSAR pour lesquels le couple (acte, intervenant, modalité collective) codé sur le RHS n'a aucune correspondance dans la table de transcodage ATIH (<code>CSAR_infos.xlsx</code>, annexe 7) — souvent parce que la modalité codée n'est pas autorisée pour cet acte (ex. "collectif" sur un acte listé individuel uniquement). Le moteur de groupage ATIH ne bloque pas ce type d'incohérence (pas d'erreur de groupage) : sans cette liste, ces actes disparaîtraient silencieusement du score de réadaptation (section 5 du TDB) — ni comptés, ni pondérés.</div></div>
  </section>
</div>
"""
